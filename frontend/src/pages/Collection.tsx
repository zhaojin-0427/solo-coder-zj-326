...
import { ERA_OPTIONS, CATEGORY_OPTIONS, STATUS_MAP, HERITAGE_TASK_STATUS_MAP, HERITAGE_TASK_STATUS_BADGE } from '@/types';
        ...
        <div className="fixed inset-0 z-50 flex justify-end" onClick={closeDetail}>
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              {(currentTerm.related_task_count ?? 0) > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-ink-500 mb-2 flex items-center gap-1.5">
                  ...
                  <div className="bg-ochre-50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-ink-700">共 {currentTerm.related_task_count} 个任务</span>
                      {currentTerm.latest_task_status && (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium ${HERITAGE_TASK_STATUS_BADGE[currentTerm.latest_task_status.status] || 'bg-cream-200 text-ink-600'}`}>
                          {HERITAGE_TASK_STATUS_MAP[currentTerm.latest_task_status.status] || currentTerm.latest_task_status.status}
                        </span>
                      )}
                    </div>
                    {currentTerm.latest_task_status && (
                      <p className="text-xs text-ink-500 truncate">{currentTerm.latest_task_status.title}</p>
                    )}
                    <button
